...
# Prioritize DIRECT_DATABASE_URL for backups since it's a direct PostgreSQL connection
# DATABASE_URL might be a Prisma Accelerate URL which won't work with pg_dump
...
if (-not $databaseUrl) {
    Write-Host "Error: DIRECT_DATABASE_URL or DATABASE_URL environment variable not found!" -ForegroundColor Red
    Write-Host "Please set one of these environment variables or create a .env file." -ForegroundColor Yellow
    Write-Host "Note: For backups, DIRECT_DATABASE_URL is preferred (direct PostgreSQL connection)." -ForegroundColor Yellow
    exit 1
}
...
# Check if this is a Prisma Accelerate URL (won't work with pg_dump)
if ($databaseUrl -match '^prisma://') {
    Write-Host "Error: Prisma Accelerate URL detected!" -ForegroundColor Red
    Write-Host "The DATABASE_URL appears to be a Prisma Accelerate URL (prisma://...)" -ForegroundColor Yellow
    Write-Host "Backups require a direct PostgreSQL connection string." -ForegroundColor Yellow
    Write-Host "" -ForegroundColor Yellow
    Write-Host "Please use DIRECT_DATABASE_URL instead, which should be in the format:" -ForegroundColor Yellow
    Write-Host "  postgres://user:password@host:port/database" -ForegroundColor White
    exit 1
}
